...
def process_func(example):
    ...
    instruction = tokenizer(f"<|begin_of_text|><|start_header_id|>system<|end_header_id|>\n\nCutting Knowledge Date: March 2026\nToday Date: 16 March 2026\n\n你现在是一个知识丰富才学八斗的教授<|eot_id|><|start_header_id|>user<|end_header_id|>\n\n{example['instruction_zh'] + example['input_zh']}<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n", add_special_tokens=False)  # add_special_tokens 不在开头加 special_tokens
    response = tokenizer(f"{example['output_zh']}<|eot_id|>", add_special_tokens=False)
...
if __name__ == "__main__":
    ...
    # 将JSON文件转换为CSV文件
    df = pd.read_json('./data/data_qa.jsonl', lines=True)
